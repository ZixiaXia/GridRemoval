...
# X4
TEST_MODEL_PATH = './logs/tf1999.ckpt'
...
if __name__ == '__main__':
    ...
    with tf.Session(config=tf.ConfigProto()) as sess:
        ...
        if TEST_EXPERIMENT:
            ...
            for testImgName in glob.glob(TEST_PATH + '*.jpg'):
                ...
                '''srcoeffs = (tcA * 255 + dcA * 255,
                            (tcH * 255 + dcH * 255,
                             tcV * 255 + dcV * 255,
                             tcD * 255 + dcD * 255))'''
                srcoeffs = (dcA.astype(np.float32) * 255,
                            (dcH.astype(np.float32) * 255,
                             dcV.astype(np.float32) * 255,
                             dcD.astype(np.float32) * 255))
                ...
                cv2.imwrite(str(TEST_SAVE_PATH +
                                str(path_leaf(testImgName).split('.')[0] + '-0' + '.jpg')), sr_img)
                print (str(path_leaf(testImgName).split('.')[0] + '-0' + '.jpg'))
